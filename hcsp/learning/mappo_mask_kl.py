...
class MAPPOPolicy_mask_KL(object):
    def __init__(self, cfg, agent_spec: AgentSpec, device="cuda", mask_name=None) -> None:
        ...
        self.kl_coef = cfg.actor.kl_coef

        self.act_dim = agent_spec.action_spec.shape[-1]
        if cfg.get("agent_name"):
            self.agent_name = cfg.agent_name
            self.obs_name = ("agents", self.agent_name + "_observation")
            self.state_name = ("agents", self.agent_name + "_state")
            self.act_name = ("agents", self.agent_name + "_action")
            self.reward_name = ("agents", self.agent_name.split("_")[0] + "_reward") # different policies of one drone have the same reward name
        else:
            raise ValueError("agent_name must be specified in the config file")
    ...
    def make_actor(self):
        ...
        if cfg.get("output_dist_params", False):
            self.actor_out_keys.append(f"{self.agent_spec.name}.action_loc")
            self.actor_out_keys.append(f"{self.agent_spec.name}.action_scale")
        ...
        td_ref = self.actor_params.to_tensordict().clone()
        td_ref.apply_(lambda x: x.clone().detach())
        self.ref_actor_params = TensorDictParams(td_ref)
        for p in self.ref_actor_params.parameters():
    ...
    def make_critic(self):
        ...
        if self.cfg.critic_input == "state" and self.agent_spec.state_spec is not None: # self.cfg.critic_input="state": centralized critic
            ...
            self.critic_out_keys = [self.agent_name + "_state_value"]
            if cfg.get("rnn", None):
    ...
    def __call__(self, tensordict: TensorDict, deterministic: bool = False):
        ...
        if self.cfg.share_actor:
            ...
            with torch.no_grad():
                ref_actor_output = self.actor(
                    actor_input, self.ref_actor_params, deterministic=deterministic
                )
        else:
            actor_output = vmap(
            ...
            with torch.no_grad():
                ref_actor_output = vmap(
                    self.actor, in_dims=(1, 0), out_dims=1, randomness="different"
                )(actor_input, self.ref_actor_params, deterministic=deterministic)
        # action_output [E,A]

        tensordict.update(actor_output)
        tensordict[f"{self.agent_spec.name}.ref_action_loc"] = ref_actor_output[f"{self.agent_spec.name}.action_loc"]
        tensordict[f"{self.agent_spec.name}.ref_action_scale"] = ref_actor_output[f"{self.agent_spec.name}.action_scale"]

        kl_div = self._kl_divergence(tensordict)
        tensordict[f"{self.agent_spec.name}.kl_div"] = kl_div
    ...
    def update_critic(self, batch: TensorDict) -> Dict[str, Any]:
        ...
        values = self.value_op(critic_input)[self.agent_name + "_state_value"]
        b_values = batch[self.agent_name + "_state_value"]
    ...
    def train_op(self, tensordict: TensorDict): 
        ...
        '''
        kl_div = tensordict[f"{self.agent_spec.name}.kl_div"]
        ...
        values = tensordict[self.agent_name + "_state_value"] # [E,T,A,1]
        next_value = value_output[self.agent_name + "_state_value"].squeeze(0) # [E,A,1]
        ...
        train_info["kl_mean"] = kl_div.mean().item()
        train_info["kl_max"] = kl_div.max().item()
        if isinstance(
    ...
    def load_state_dict(self, state_dict):
        ...
        if "actor_opt" in state_dict:
            ...
            logging.info("Load actor_opt.")
        self.critic.load_state_dict(state_dict["critic"])
        if "critic_opt" in state_dict:
            self.critic_opt.load_state_dict(state_dict["critic_opt"])
            logging.info("Load critic_opt.")
    ...
    def _kl_divergence(self, tensordict: TensorDict, eps=1e-8) -> torch.Tensor:
        """
        calculate KL divergence of two CustomDiagGaussian distribution
        """
        current_loc = tensordict[f"{self.agent_spec.name}.action_loc"]          # [B,T,A,4]
        current_scale = tensordict[f"{self.agent_spec.name}.action_scale"]      # [B,T,A,4]
        ref_loc = tensordict[f"{self.agent_spec.name}.ref_action_loc"].detach()          # [B,T,A,4]
        ref_scale = tensordict[f"{self.agent_spec.name}.ref_action_scale"].detach()      # [B,T,A,4]

        eps = eps
        current_scale = current_scale + eps
        ref_scale = ref_scale + eps

        log_ratio = torch.log(ref_scale / current_scale)                    # log(σ2/σ1)
        variance_term = (current_scale**2 + (current_loc - ref_loc)**2)     # σ1² + (μ1-μ2)^2
        variance_term = variance_term / (2 * ref_scale**2)                  # / (2σ2²)
        kl_div = log_ratio + variance_term - 0.5                            # [B,T,A,4]

        kl_div = kl_div.sum(dim=-1, keepdim=True) # [B,T,A,1]
...
def make_ppo_actor(cfg, observation_spec: TensorSpec, action_spec: TensorSpec):
    ...
    return Actor(
        encoder, act_dist, rnn, output_dist_params=True
    )
