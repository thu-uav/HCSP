...
from hcsp.learning import MAPPOPolicy_mask

class MAPPOPolicy_Receive(object):
    ...
    def __init__(self, cfg, agent_spec_dict: Dict[str, AgentSpec], device="cuda") -> None:
        ...
        self.policy_FirstPass = MAPPOPolicy_mask(cfg=cfg, agent_spec=FirstPass_agent_spec, device=device, mask_name=('stats', 'Opp_Server_hit'))
    ...
    def load_state_dict(self, state_dict, player: str):
        if player == "Opp_Server":
            ...
            raise ValueError("player should be 'Opp_Server', 'Opp_Server_hover', 'FirstPass_goto' or 'FirstPass'")
