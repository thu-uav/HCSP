...
class Pass_hover(IsaacEnv):
    ...
    def __init__(self, cfg, headless):
        ...
        self.done_ball_hit_ground = cfg.task.done_ball_hit_ground
        ...
        self.FirstPass_policy_start_time = "Opp_Att_hit"
    ...
    def _set_specs(self):
        ...
        if self.stats_cfg.get("complete_reward_stats", False):
            ...
            _stats_spec.set("reward_effort", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("reward_action_smoothness", UnboundedContinuousTensorSpec(1))

            _stats_spec.set("penalty_drone_pass_the_net", UnboundedContinuousTensorSpec(1))
    ...
    def debug_draw_turn(self):
        ...
        if turn.item() == 0:
            ...
            colors_turn = [(0, 0, 1, 1)]
            if self.FirstPass_policy_start_time == "Opp_Att_hit":
                logging.info("Central env turn: Opp_Attacker turn")
            elif self.FirstPass_policy_start_time == "Opp_SecPass_hit":                
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        self.FirstPass_policy_start_time == "Opp_SecPass_hit":
        0: all hover
        1: Opp_SecPass turn
    ...
    def _compute_reward_and_done(self):
        ...
        # reward 1: hover
        rpos_hover = torch.norm(self.drone.pos[:, 2, :].unsqueeze(1) - self.FirstPass_hover_pos_after_hit, dim=-1) # (E,1)
        rheading = torch.norm(self.drone.heading[:, 2, :].unsqueeze(1) - torch.tensor([1., 0., 0.], device=self.device).unsqueeze(0).unsqueeze(0), dim=-1) # (E,1)
        
        distance = torch.norm(torch.cat([rpos_hover, rheading], dim=-1), dim=-1).unsqueeze(-1) # (E,1)
        reward_pose = 1.0 / (1.0 + torch.square(1.2 * distance))
        
        reward_up = 1.0 * torch.square((self.drone.up[:, 2, 2] + 1) / 2).unsqueeze(-1)
        
        spinnage = torch.square(self.drone.vel[:, 2, -1]).unsqueeze(-1)
        reward_spin = 1.0 / (1.0 + torch.square(spinnage))

        reward_effort = 0.1 * torch.exp(-self.effort[:, 2]).unsqueeze(-1)
        reward_action_smoothness = 0.01 * torch.exp(-self.drone.throttle_difference[:, 2]).unsqueeze(-1)
        
        reward_hover = 3 * self.FirstPass_hit * (
            reward_pose 
        ...
        terminated = (
            (self.drone.pos[..., 2] < 0.35).any(dim=1).unsqueeze(-1) | 
        ...
        _penalty_drone_pass_the_net_coeff = 100.0
        drone_pass_the_net = calculate_drone_pass_the_net(self.drone.pos[:, 2, :].unsqueeze(1)) # (E,1)
        penalty_drone_pass_the_net = self.FirstPass_hit * _penalty_drone_pass_the_net_coeff * drone_pass_the_net.float() # (E,1)

        _penalty_drone_hit_the_ground_coeff = 100.0
        drone_hit_the_ground = (self.drone.pos[:, 2, 2].unsqueeze(-1) < 0.35)
        penalty_drone_hit_the_ground = self.FirstPass_hit * _penalty_drone_hit_the_ground_coeff * drone_hit_the_ground.float() # (E,1)
        
        reward_end = - penalty_drone_pass_the_net - penalty_drone_hit_the_ground # (E,1)
        ...
        if self.stats_cfg.get("complete_reward_stats", False):
            ...
            self.stats["penalty_drone_pass_the_net"].add_(penalty_drone_pass_the_net)
            self.stats["penalty_drone_hit_the_ground"].add_(penalty_drone_hit_the_ground)
