...
class Receive(IsaacEnv):
    ...
    def _set_specs(self):
        ...
        if self.stats_cfg.get("complete_reward_stats", False):
            ...
            _stats_spec.set("reward_Server_hit_pos", UnboundedContinuousTensorSpec(1))
    ...
    def _compute_reward_and_done(self):
        ...
        reward_hit = (self.FirstPass_turn == 1).unsqueeze(-1) * _reward_hit_coeff * torch.where(racket_hit_ball[:, 1], 1.0, 0.0).unsqueeze(-1)
        ...
        self.stats["FirstPass_hit_pos_z"] += self.FirstPass_pos[..., 2] * racket_hit_ball[:, 1].unsqueeze(-1)
        ...
        reward_my_turn = (1 - self.ball_already_hit_the_ground) * (reward_rpos + reward_hit  - penalty_pos_x - penalty_hit_pos_z - penalty_yaw - penalty_roll) # (E,1)
        ...
        _reward_rpos_anchor_coeff = 50.0
        ...
        FirstPass_wrong_hit_racket = wrong_hit_racket[:, 1].unsqueeze(-1)
        FirstPass_wrong_hit_turn = ((self.FirstPass_turn == 0) & racket_hit_ball[:, 1]).unsqueeze(-1)
        ...
        FirstPass_reward = torch.where(self.Opp_Server_hit.bool(), FirstPass_reward_2, FirstPass_reward_1)
