...
class Attack(IsaacEnv):
    ...
    def __init__(self, cfg, headless):
        ...
        self.Att_hit_t = torch.zeros((self.num_envs, 1), device=self.device)
    ...
    def _set_specs(self):
        ...
        if self.stats_cfg.get("complete_reward_stats", False):
            ...
            _stats_spec.set("reward_hit_pos_z", UnboundedContinuousTensorSpec(1))
            ...
            _stats_spec.set("reward_ball_vel_x", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("reward_ball_vel_z", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("reward_rpos_anchor", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("reward_ball_hit_the_ground", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("reward_ball_in_side", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("reward_ball_hit_the_ground_vel", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("penalty_end_wrong_hit", UnboundedContinuousTensorSpec(1))
            _stats_spec.set("penalty_ball_hit_the_net", UnboundedContinuousTensorSpec(1))
    ...
    def debug_draw_Att_attacking_target(self):
        ...
        if attacking_target.item() == 0:
            points = torch.tensor([0., -2., 0.]).to(self.device) + ori
        else:
            points = torch.tensor([0., 2., 0.]).to(self.device) + ori
        points = [points.tolist()]
    ...
    def _compute_reward_and_done(self):
        ...
        # ball_near_ground = (self.ball_pos[..., 2] < 0.2) # (E,1)
        # drone_near_ball = ((self.drone.pos - self.ball_pos).norm(dim=-1) < 0.5) # (E, 2)
        # ball_vel_z_change = ((self.ball_vel[..., 2] - self.ball_last_vel[..., 2]) > -0.196) # (E, 1)
        # hit_1 = drone_near_ball & ball_vel_z_change # (E, 2) # 
        # hit_ground_1 = ball_near_ground & ball_vel_z_change # (E, 1)
        # ball_vel_x_y_change = ((self.ball_vel[..., :2] - self.ball_init_vel[..., :2]).norm(dim=-1) > 0.5) # (E, 1)
        # hit_2 = drone_near_ball & ball_vel_x_y_change & ~torch.cat((self.SecPass_hit, self.Att_hit), dim=-1).bool() # (E, 2) # 
        # hit_ground_2 = ball_near_ground & ball_vel_x_y_change # (E, 1)
        # hit = hit_1 | hit_2 & ~hit_ground_1 & ~hit_ground_2 & ~self.ball_already_hit_the_ground.bool() # (E, 2) drone hit ball
        ...
        reward_hit = (self.Att_turn == 1).unsqueeze(-1) * 20 * torch.where(hit[:, 1].unsqueeze(-1), 1.0, 0.0)
        
        reward_hit_pos_z = 400 * (self.Att_turn == 1).unsqueeze(-1) * 1.0 / (1.0 + 2 * torch.abs(self.ball_pos[..., 2] - 3.6)) * torch.where(self.Att_hit.bool(), 1.0, 0.0)
        # reward_hit_at_highest_point = 200 * (self.Att_turn == 1).unsqueeze(-1) * 1.0 / (1.0 + torch.square(self.ball_vel[..., 2])) * torch.where(hit[:, 1].unsqueeze(-1), 1.0, 0.0)
        reward_rpos = (self.Att_turn == 1).unsqueeze(-1) * 1.0 / (1.0 + torch.square(1.2 * torch.norm(self.Att_rpos, dim=-1)))
        penalty_yaw = (self.Att_turn == 1).unsqueeze(-1) * 2.0 * self.Att_yaw.abs()
        penalty_roll = (self.Att_turn == 1).unsqueeze(-1) * 2.0 * self.Att_roll.abs()
        
        reward_my_turn = reward_rpos + reward_hit + reward_hit_pos_z - penalty_yaw - penalty_roll # (E,1)


        # reward 3: Reward after my turn
        reward_ball_vel_x = self.Att_hit * 1. * (self.ball_vel[..., 0] < 0) * (1 - self.ball_already_hit_the_ground)
        
        rpos_anchor = self.ball_pos - self.ball_anchor
        reward_rpos_anchor = self.Att_hit * 150. / (1.0 + torch.square(1.2 * torch.norm(rpos_anchor, dim=-1))) * (1 - self.ball_already_hit_the_ground)

        close_to_hit = (self.Att_hit_t > 0) & ((self.progress_buf.unsqueeze(-1) - self.Att_hit_t) < 5)
        
        _reward_ball_vel_z_coeff = 60.0
        reward_ball_vel_z = _reward_ball_vel_z_coeff * self.Att_hit * close_to_hit * (self.ball_vel[..., 2] < 0) * (5 - ((self.progress_buf.unsqueeze(-1) - self.Att_hit_t))) * (1 - self.ball_already_hit_the_ground)
        # reward_ball_vel_z = _reward_ball_vel_z_coeff * self.Att_hit * close_to_hit * (7.0 - self.ball_vel[..., 2]) * ((self.progress_buf.unsqueeze(-1) - self.Att_hit_t) == 1) * (1 - self.ball_already_hit_the_ground)
        
        reward_after_my_turn = reward_ball_vel_x + reward_rpos_anchor + reward_ball_vel_z # (E,1)
        ...
        reward_ball_hit_the_ground = 5 * self.ball_first_hit_the_ground
        ...
        reward_ball_in_side = 10 * in_side 
        reward_ball_hit_the_ground_vel = in_side * 20 * self.ball_vel.norm(dim=-1)
        penalty_end_wrong_hit = 10 * ((self.Att_turn == 1).unsqueeze(-1) & hit[:, 1].unsqueeze(-1))
        penalty_end_wrong_hit = torch.where(
            done, 
            penalty_end_wrong_hit, 
            torch.zeros_like(done, device=self.device)
        ) # (E,1)
        penalty_ball_hit_the_net = 10 * ball_hit_the_net
        
        reward_end = reward_ball_hit_the_ground + reward_ball_in_side + reward_ball_hit_the_ground_vel - penalty_end_wrong_hit - penalty_ball_hit_the_net # (E,1)
        if self.done_Att_pass_the_net:
            reward_end -= penalty_Att_pass_the_net

        # Overall reward
        reward = reward_my_turn + reward_after_my_turn + reward_end  # (E,1)

        Att_reward_1 = torch.zeros((self.num_envs, 1), device=self.device) # already trained GoTo policy
        Att_reward_2 = reward # attack policy to be trained
        ...
        self.Att_hit_t[Att_hit_index] = self.progress_buf[Att_hit_index].unsqueeze(-1)
        ...
        if self.stats_cfg.get("complete_reward_stats", False):
            ...
            self.stats["reward_hit_pos_z"].add_(reward_hit_pos_z)
            self.stats["reward_rpos"].add_(reward_rpos)
            self.stats["penalty_yaw"].add_(penalty_yaw)
            self.stats["penalty_roll"].add_(penalty_roll)
            self.stats["reward_ball_vel_x"].add_(reward_ball_vel_x)
            self.stats["reward_ball_vel_z"].add_(reward_ball_vel_z)
            self.stats["reward_rpos_anchor"].add_(reward_rpos_anchor)
            self.stats["reward_ball_hit_the_ground"].add_(reward_ball_hit_the_ground)
            self.stats["reward_ball_in_side"].add_(reward_ball_in_side)
            self.stats["reward_ball_hit_the_ground_vel"].add_(reward_ball_hit_the_ground_vel)
            self.stats["penalty_end_wrong_hit"].add_(penalty_end_wrong_hit)
            self.stats["penalty_ball_hit_the_net"].add_(penalty_ball_hit_the_net)
