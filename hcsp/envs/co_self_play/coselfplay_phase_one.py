...
from hcsp.utils.volleyball.common import (
    ...
    determine_game_result_3v3_strict
)
...
class Coselfplay_Phase_one(IsaacEnv):
    ...
    def __init__(self, cfg, headless):
        ...
        self.ball_from_near_side_to_far_side = torch.zeros((self.num_envs, 1), device=self.device, dtype=torch.bool)
        self.ball_from_far_side_to_near_side = torch.zeros((self.num_envs, 1), device=self.device, dtype=torch.bool)
        ...
        # for KL penalty finetuning LLS

        self.SecPass_kl_div = torch.zeros((self.num_envs, 1, 1), device=self.device)
        self.kl_coef = cfg.algo.actor.kl_coef
    ...
    def _set_specs(self):
        ...
        self.action_spec = CompositeSpec({
            "agents": CompositeSpec({
                ...
                "high_level_action": MultiDiscreteTensorSpec(nvec=(6, 3, 4), shape=(2, 3), dtype=torch.long, device=self.device)
            })
        }).expand(self.num_envs).to(self.device)
        ...
        _stats_spec = CompositeSpec({
            ...
            "FirstPass_train_flag": UnboundedContinuousTensorSpec(1),
            "FirstPass_hover_train_flag": UnboundedContinuousTensorSpec(1),
            "SecPass_train_flag": UnboundedContinuousTensorSpec(1),
            "SecPass_hover_train_flag": UnboundedContinuousTensorSpec(1),
            "Att_train_flag": UnboundedContinuousTensorSpec(1),
            "Att_hover_train_flag": UnboundedContinuousTensorSpec(1),
        ...
        if self.stats_cfg.get("complete_reward_stats", False):
            ...
            _stats_spec["return_SecPass"] = UnboundedContinuousTensorSpec(1)

            _stats_spec["true_SecPass_hit_reward"] = UnboundedContinuousTensorSpec(1)
            _stats_spec["SecPass_kl_div_penalty"] = UnboundedContinuousTensorSpec(1)
            _stats_spec["strict_game_result_reward"] = UnboundedContinuousTensorSpec(1)
    ...
    def debug_draw_FirstPass_turn(self):
        env_id = self.central_env_idx
        ori = self.envs_positions[env_id].detach()
        
        point_FirstPass_turn = torch.tensor([4., 0., 0.]).to(self.device) + ori
        point_FirstPass_train = torch.tensor([5., 0., 0.]).to(self.device) + ori
        points = [point_FirstPass_turn.tolist(), point_FirstPass_train.tolist()]
        if self.FirstPass_turn[env_id].item() == 1:
            color_FirstPass_turn = (0, 1, 0, 1)
        else:
            color_FirstPass_turn = (1, 0, 0, 1)
        
        FirstPass_train_flag = (
            (self.info["high_level_action"][:, 0, 0] == 3) |
            (self.info["high_level_action"][:, 0, 0] == 4)
        ) # (E,)
        if FirstPass_train_flag[env_id].item() == True:
            color_FirstPass_train = (0, 1, 0, 1)
        else:
            color_FirstPass_train = (1, 0, 0, 1)

        colors = [color_FirstPass_turn, color_FirstPass_train]
        sizes = [20., 20.]

        self.draw.draw_points(points, colors, sizes)
    

    def debug_draw_SecPass_turn(self):
        env_id = self.central_env_idx
        ori = self.envs_positions[env_id].detach()

        point_SecPass_turn = torch.tensor([4., -2., 0.]).to(self.device) + ori
        point_SecPass_train = torch.tensor([5., -2., 0.]).to(self.device) + ori
        points = [point_SecPass_turn.tolist(), point_SecPass_train.tolist()]
        if self.SecPass_turn[env_id].item() == 1:
            color_SecPass_turn = (0, 1, 0, 1)
        else:
            color_SecPass_turn = (1, 0, 0, 1)
        
        SecPass_train_flag = (
            (self.info["high_level_action"][:, 0, 1] == 1)
        ) # (E,)
        if SecPass_train_flag[env_id].item() == True:
            color_SecPass_train = (0, 1, 0, 1)
        else:
            color_SecPass_train = (1, 0, 0, 1)

        colors = [color_SecPass_turn, color_SecPass_train]
        sizes = [20., 20.]

        self.draw.draw_points(points, colors, sizes)
    

    def debug_draw_Att_turn(self):
        env_id = self.central_env_idx
        ori = self.envs_positions[env_id].detach()

        point_Att_turn = torch.tensor([4., 2., 0.]).to(self.device) + ori
        point_Att_train = torch.tensor([5., 2., 0.]).to(self.device) + ori
        points = [point_Att_turn.tolist(), point_Att_train.tolist()]
        if self.Att_turn[env_id].item() == 1:
            color_Att_turn = (0, 1, 0, 1)
        else:
            color_Att_turn = (1, 0, 0, 1)
        
        Att_train_flag = (
            (self.info["high_level_action"][:, 0, 2] == 1) |
            (self.info["high_level_action"][:, 0, 2] == 2)
        ) # (E,)
        if Att_train_flag[env_id].item() == True:
            color_Att_train = (0, 1, 0, 1)
        else:
            color_Att_train = (1, 0, 0, 1)

        colors = [color_Att_turn, color_Att_train]
        sizes = [20., 20.]
    ...
    def debug_draw_turn(self):
        ...
        point = torch.tensor([3., 0., 0.]).to(self.device) + ori
        point = [point.tolist()]

        if self.info["switch_turn"][env_id].item() == True:
        ...
        self.draw.draw_points(point, color, sizes)
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        self.ball_from_near_side_to_far_side[env_ids] = False
        self.ball_from_far_side_to_near_side[env_ids] = False
        ...
        if (env_ids == self.central_env_idx).any() and self._should_render(0):
            ...
            # self.debug_draw_FirstPass_turn()
            # self.debug_draw_SecPass_turn()
            # self.debug_draw_Att_turn()


    def _pre_sim_step(self, tensordict: TensorDictBase):
        # kl_divergence
        self.SecPass_kl_div = tensordict["SecPass.kl_div"] # (E,1,1)

        # high-level action
        high_level_action = tensordict[("agents", "high_level_action")] # (E,2,3)

        # if SecPass hit, opponent team all hover
        high_level_action[:, 1, 0] = torch.where(
            self.SecPass_already_hit.squeeze(-1).bool(), # (E,)
            1,
            high_level_action[:, 1, 0]
        )
        high_level_action[:, 1, 1] = torch.where(
            self.SecPass_already_hit.squeeze(-1).bool(), # (E,)
            0,
        ...
        self.stats["Opp_FirstPass_high_level_0"] += self.info["switch_turn"] * (high_level_action[:, 1, 0] == 0).unsqueeze(-1) * self.is_rally.unsqueeze(-1)
        self.stats["Opp_FirstPass_high_level_1"] += self.info["switch_turn"] * (high_level_action[:, 1, 0] == 1).unsqueeze(-1) * self.is_rally.unsqueeze(-1)
        self.stats["Opp_FirstPass_high_level_2"] += self.info["switch_turn"] * (high_level_action[:, 1, 0] == 2).unsqueeze(-1) * self.is_rally.unsqueeze(-1)
        self.stats["Opp_FirstPass_high_level_3"] += self.info["switch_turn"] * (high_level_action[:, 1, 0] == 3).unsqueeze(-1) * self.is_rally.unsqueeze(-1)
        self.stats["Opp_FirstPass_high_level_4"] += self.info["switch_turn"] * (high_level_action[:, 1, 0] == 4).unsqueeze(-1) * self.is_rally.unsqueeze(-1)
        self.stats["Opp_FirstPass_high_level_5"] += self.info["switch_turn"] * (high_level_action[:, 1, 0] == 5).unsqueeze(-1) * self.is_rally.unsqueeze(-1)
        self.stats["Opp_SecPass_high_level_0"] += self.info["switch_turn"] * (high_level_action[:, 1, 1] == 0).unsqueeze(-1)
        self.stats["Opp_SecPass_high_level_1"] += self.info["switch_turn"] * (high_level_action[:, 1, 1] == 1).unsqueeze(-1)
        self.stats["Opp_SecPass_high_level_2"] += self.info["switch_turn"] * (high_level_action[:, 1, 1] == 2).unsqueeze(-1)
        self.stats["Opp_Att_high_level_0"] += self.info["switch_turn"] * (high_level_action[:, 1, 2] == 0).unsqueeze(-1)
        self.stats["Opp_Att_high_level_1"] += self.info["switch_turn"] * (high_level_action[:, 1, 2] == 1).unsqueeze(-1)
        self.stats["Opp_Att_high_level_2"] += self.info["switch_turn"] * (high_level_action[:, 1, 2] == 2).unsqueeze(-1)
        self.stats["Opp_Att_high_level_3"] += self.info["switch_turn"] * (high_level_action[:, 1, 2] == 3).unsqueeze(-1)
        ...
        Opp_FirstPass_real_action = torch.where(
            (
                (self.info["high_level_action"][:, 1, 0] == 0) |
                (self.info["high_level_action"][:, 1, 0] == 1) |
                (self.info["high_level_action"][:, 1, 0] == 2)
            ).unsqueeze(-1).unsqueeze(-1), # (E,1,1)
            Opp_FirstPass_goto_action,
            torch.where(
                (
                    (self.info["high_level_action"][:, 1, 0] == 3) |
                    (self.info["high_level_action"][:, 1, 0] == 4)
                ).unsqueeze(-1).unsqueeze(-1), # (E,1,1)
        ...
        Opp_SecPass_real_action = torch.where(
            (self.info["high_level_action"][:, 1, 1] == 0).unsqueeze(-1).unsqueeze(-1), # (E,1,1)
            Opp_SecPass_goto_action,
            torch.where(
                (self.info["high_level_action"][:, 1, 1] == 1).unsqueeze(-1).unsqueeze(-1), # (E,1,1)
                Opp_SecPass_action,
        ...
        Opp_Att_real_action = torch.where(
            (self.info["high_level_action"][:, 1, 2] == 0).unsqueeze(-1).unsqueeze(-1), # (E,1,1)
            Opp_Att_goto_action,
            torch.where(
                (
                    (self.info["high_level_action"][:, 1, 2] == 1) |
                    (self.info["high_level_action"][:, 1, 2] == 2)
                ).unsqueeze(-1).unsqueeze(-1), # (E,1,1)
                Opp_Att_action,
    ...
    def update_ball_pass_net(self):
        ...
        self.ball_from_near_side_to_far_side = (ball_at_far_side & ~self.ball_side).unsqueeze(-1) # (E,1)
        self.FirstPass_already_hit = torch.where(
            self.ball_from_near_side_to_far_side,
            False,
        ...
        self.ball_from_far_side_to_near_side = (~ball_at_far_side & self.ball_side).unsqueeze(-1) # (E,1)
        self.Opp_FirstPass_already_hit = torch.where(
            self.ball_from_far_side_to_near_side,
            False,
        ...
        self.ball_pass_net = (self.ball_from_near_side_to_far_side | self.ball_from_far_side_to_near_side).squeeze(-1) # (E,)
        ...
        if self._should_render(0):
            if self.ball_from_near_side_to_far_side[self.central_env_idx].item() == 1:
                logging.info("Ball passed the net from near side to far side.")
            elif self.ball_from_far_side_to_near_side[self.central_env_idx].item() == 1:
    ...
    def compute_attacking_target_and_goto_pos(self):
        ...
        self.Opp_FirstPass_goto_pos_before_hit = torch.where(
            (self.info["high_level_action"][:, 1, 0] == 0).unsqueeze(-1).unsqueeze(-1),
            self.Opp_FirstPass_goto_pos_left_before_hit,
            torch.where(
                (self.info["high_level_action"][:, 1, 0] == 1).unsqueeze(-1).unsqueeze(-1),
                self.Opp_FirstPass_hover_pos_after_hit,
                torch.where(
                    (self.info["high_level_action"][:, 1, 0] == 2).unsqueeze(-1).unsqueeze(-1),
        ...
        self.Opp_FirstPass_obs_attacking_target = torch.where(
            (self.info["high_level_action"][:, 1, 0] == 3).unsqueeze(-1).unsqueeze(-1),
            self.FirstPass_obs_left_attacking_target,
            torch.where(
                (self.info["high_level_action"][:, 1, 0] == 4).unsqueeze(-1).unsqueeze(-1),
    ...
    def compute_turn(self):
        ...
        self.Opp_SecPass_turn = torch.where(
            (self.info["high_level_action"][:, 1, 1] == 1), # (E,)
            1,
            0
        )
        ...
        self.SecPass_turn = torch.where(
            (self.info["high_level_action"][:, 0, 1] == 1), # (E,)
            1,
            0
        )
    ...
    def _compute_state_and_obs(self, is_step=False):
        ...
        # save important information for low-level skills reward computing
    ...
    def _compute_reward_and_done(self):
        ...
        true_Opp_FirstPass_hit = Opp_FirstPass_hit & ((self.info["high_level_action"][:, 1, 0] == 3) | (self.info["high_level_action"][:, 1, 0] == 4))
        true_Opp_SecPass_hit = Opp_SecPass_hit & (self.info["high_level_action"][:, 1, 1] == 1)
        true_Opp_Att_hit = Opp_Att_hit & ((self.info["high_level_action"][:, 1, 2] == 1) | (self.info["high_level_action"][:, 1, 2] == 2))
        ...
        near_side_win, far_side_win, both_win = determine_game_result_3v3_strict(
            self.drone.pos, 
        ...
        high_level_reward = reward_hit + reward_win

        game_reward_near_side = reward_win[:, 0].unsqueeze(-1) + reward_hit[:, 0].unsqueeze(-1)
        ...
        #     Att_hit
        # ).unsqueeze(-1) # (E, 1)
        # FirstPass_reward = game_reward_near_side # (E, 1)
        # self.stats["FirstPass_train_flag"] = FirstPass_train_flag.float()

        # # FirstPass_hover
        # FirstPass_hover_train_flag = (
        #     (self.info["high_level_action"][:, 0, 0] == 5) |
        #     done.squeeze(-1) |
        #     FirstPass_hit |
        #     SecPass_hit |
        #     Att_hit
        # ).unsqueeze(-1)
        # FirstPass_hover_reward = game_reward_near_side
        # self.stats["FirstPass_hover_train_flag"] = FirstPass_hover_train_flag.float()
        ...
        self.stats["SecPass_train_flag"] = SecPass_train_flag.float()

        _true_SecPass_hit_reward_coef = 5.0
        true_SecPass_hit_reward = _true_SecPass_hit_reward_coef * true_SecPass_hit.unsqueeze(-1) # (E, 1)

        strict_game_result_reward = reward_win[:, 0].unsqueeze(-1)

        SecPass_kl_div_penalty = self.kl_coef * self.SecPass_kl_div.squeeze(1)
        
        SecPass_reward = true_SecPass_hit_reward + strict_game_result_reward - SecPass_kl_div_penalty
        
        # # SecPass_hover
        # SecPass_hover_train_flag = (
        #     (self.info["high_level_action"][:, 0, 1] == 2) |
        #     done.squeeze(-1) |
        #     FirstPass_hit |
        #     SecPass_hit |
        #     Att_hit
        # ).unsqueeze(-1)
        # SecPass_hover_reward = game_reward_near_side
        # self.stats["SecPass_hover_train_flag"] = SecPass_hover_train_flag.float()
        ...
        #     Att_hit
        # ).unsqueeze(-1)
        # Att_reward = game_reward_near_side
        # self.stats["Att_train_flag"] = Att_train_flag.float()

        # # Att_hover
        # Att_hover_train_flag = (
        #     (self.info["high_level_action"][:, 0, 2] == 3) |
        #     done.squeeze(-1) |
        #     FirstPass_hit |
        #     SecPass_hit |
        #     Att_hit
        # ).unsqueeze(-1)
        # Att_hover_reward = game_reward_near_side
        # self.stats["Att_hover_train_flag"] = Att_hover_train_flag.float()
        ...
        self.stats["Opp_Att_hit_left"][done_idx] /= (self.stats["num_Opp_SecPass_hit"][done_idx] + 1e-6)
        self.stats["Opp_Att_hit_right"][done_idx] /= (self.stats["num_Opp_SecPass_hit"][done_idx] + 1e-6)
        self.stats["Opp_Att_hit_empty_spot"][done_idx] /= (self.stats["num_Opp_SecPass_hit"][done_idx] + 1e-6)
        self.stats["Opp_Att_hit_wrong_spot"][done_idx] /= (self.stats["num_Opp_SecPass_hit"][done_idx] + 1e-6)
        
        self.stats["FirstPass_goto_left"][done_idx] /= (self.stats["num_Opp_SecPass_hit"][done_idx] + 1e-6)
        self.stats["FirstPass_goto_right"][done_idx] /= (self.stats["num_Opp_SecPass_hit"][done_idx] + 1e-6)

        self.stats["Att_hit_left"][done_idx] /= (self.stats["num_SecPass_hit"][done_idx] + 1e-6)
        self.stats["Att_hit_right"][done_idx] /= (self.stats["num_SecPass_hit"][done_idx] + 1e-6)
        self.stats["Att_hit_empty_spot"][done_idx] /= (self.stats["num_SecPass_hit"][done_idx] + 1e-6)
        self.stats["Att_hit_wrong_spot"][done_idx] /= (self.stats["num_SecPass_hit"][done_idx] + 1e-6)
        
        self.stats["Opp_FirstPass_goto_left"][done_idx] /= (self.stats["num_SecPass_hit"][done_idx] + 1e-6)
        self.stats["Opp_FirstPass_goto_right"][done_idx] /= (self.stats["num_SecPass_hit"][done_idx] + 1e-6)
        ...
        if self.stats_cfg.get("complete_reward_stats", False):
            ...
            self.stats["return_SecPass"].add_(SecPass_reward * self.stats["SecPass_train_flag"])
            self.stats["true_SecPass_hit_reward"].add_(true_SecPass_hit_reward * self.stats["SecPass_train_flag"])
            self.stats["SecPass_kl_div_penalty"].add_(SecPass_kl_div_penalty * self.stats["SecPass_train_flag"])
            self.stats["strict_game_result_reward"].add_(strict_game_result_reward * self.stats["SecPass_train_flag"]) 
