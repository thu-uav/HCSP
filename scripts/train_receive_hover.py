...
@hydra.main(version_base=None, config_path=CONFIG_PATH, config_name="train_receive_hover")
def main(cfg: DictConfig):
    ...
    algos = {
        "mappo_receive_hover": MAPPOPolicy_Receive_hover,
    }
    ...
    if only_eval == False: 
        ...
        for i, data in enumerate(pbar):
            ...
            if save_interval > 0 and i % save_interval == 0:
                if hasattr(policy, "state_dict"):
                    ckpt_path = os.path.join(run.dir, f"checkpoint_{collector._frames}_FirstPass_hover.pt")
                    logging.info(f"Save checkpoint to {str(ckpt_path)}")
                    torch.save(policy.state_dict(player="FirstPass_hover"), ckpt_path)
        ...
        if hasattr(policy, "state_dict"):
            ckpt_path = os.path.join(run.dir, f"checkpoint_final_FirstPass_hover.pt")
            logging.info(f"Save checkpoint to {str(ckpt_path)}")
            torch.save(policy.state_dict(player="FirstPass_hover"), ckpt_path)
