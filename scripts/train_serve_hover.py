...
# change agent_spec
# change load_policy
# change save_checkpoint

@hydra.main(version_base=None, config_path=CONFIG_PATH, config_name="train_serve_hover")
def main(cfg: DictConfig):
    ...
    Server_agent_spec: AgentSpec = env.agent_spec["Server"]
    Server_hover_agent_spec: AgentSpec = env.agent_spec["Server_hover"]

    agent_spec_dict = {}
    agent_spec_dict["Server"] = Server_agent_spec
    agent_spec_dict["Server_hover"] = Server_hover_agent_spec

    policy = algos[cfg.algo.name.lower()](
        cfg.algo, agent_spec_dict=agent_spec_dict, device="cuda"
    )

    if cfg.get("Server_policy_checkpoint_path") is not None:
        policy.load_state_dict(torch.load(cfg.Server_policy_checkpoint_path), player="Server")
        print(f"Load Server policy from {cfg.Server_policy_checkpoint_path}")
    if cfg.get("Server_hover_policy_checkpoint_path") is not None:
        policy.load_state_dict(torch.load(cfg.Server_hover_policy_checkpoint_path), player="Server_hover")
        print(f"Load Server_hover policy from {cfg.Server_hover_policy_checkpoint_path}")
    ...
    if only_eval == False: 
        ...
        for i, data in enumerate(pbar):
            ...
            if save_interval > 0 and i % save_interval == 0:
                if hasattr(policy, "state_dict"):
                    ckpt_path = os.path.join(run.dir, f"checkpoint_{collector._frames}_Server_hover.pt")
                    logging.info(f"Save checkpoint to {str(ckpt_path)}")
                    torch.save(policy.state_dict(player="Server_hover"), ckpt_path)
        ...
        if hasattr(policy, "state_dict"):
            ckpt_path = os.path.join(run.dir, f"checkpoint_final_Server_hover.pt")
            logging.info(f"Save checkpoint to {str(ckpt_path)}")
            torch.save(policy.state_dict(player="Server_hover"), ckpt_path)
