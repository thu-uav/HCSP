...
@hydra.main(version_base=None, config_path=CONFIG_PATH, config_name="train_set_hover")
def main(cfg: DictConfig):
    ...
    algos = {
        "mappo_set_hover": MAPPOPolicy_Set_hover,
    }
    ...
    if only_eval == False: 
        ...
        for i, data in enumerate(pbar):
            ...
            if save_interval > 0 and i % save_interval == 0:
                if hasattr(policy, "state_dict"):
                    ckpt_path = os.path.join(run.dir, f"checkpoint_{collector._frames}_SecPass_hover.pt")
                    logging.info(f"Save checkpoint to {str(ckpt_path)}")
                    torch.save(policy.state_dict(player="SecPass_hover"), ckpt_path)
        ...
        if hasattr(policy, "state_dict"):
            ckpt_path = os.path.join(run.dir, f"checkpoint_final_SecPass_hover.pt")
            logging.info(f"Save checkpoint to {str(ckpt_path)}")
            torch.save(policy.state_dict(player="SecPass_hover"), ckpt_path)
