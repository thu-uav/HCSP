...
from omni_drones.learning import PSROPolicy_high_level, PSROPolicy
...
@torch.no_grad()
def evaluate(env: TransformedEnv, policy: PSROPolicy_high_level):
    """
...
def _get_psro_high_level_policy(cfg: DictConfig, env: TransformedEnv):
    algos = {
        "psro_high_level": PSROPolicy_high_level,
    }
...
def _load_low_level_policy(cfg: DictConfig, policy: PSROPolicy_high_level):
...
def _load_high_level_policy(cfg: DictConfig, policy: PSROPolicy_high_level):
    """
...
@torch.no_grad()
def eval_win_rate(env: TransformedEnv, policy: PSROPolicy_high_level):
    high_level_policy = policy.policy_high_level
    env.reset()
    env.eval()
    high_level_policy.eval_payoff = True
...
def train(
    ...
    policy: PSROPolicy_high_level = _get_psro_high_level_policy(cfg, env)
...
@hydra.main(version_base=None, config_path=CONFIG_PATH, config_name="train_psro_high_level")
