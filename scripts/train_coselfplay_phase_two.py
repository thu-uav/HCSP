...
from omni_drones.learning import PSROPolicy_coselfplay_phase_two, PSROPolicy
...
@torch.no_grad()
def evaluate(env: TransformedEnv, policy: PSROPolicy_coselfplay_phase_two):
    """
    Use the latest strategies of both teams to evaluate the policy
    """
    high_level_old_policy = policy.policy_high_level_old
    ...
    high_level_old_policy.set_latest_strategy()
...
# @torch.no_grad()
# def evaluate_share_population(env: TransformedEnv, policy: PSROPolicy_high_level):
#     """
...
def _get_psro_high_level_policy(cfg: DictConfig, env: TransformedEnv):
    algos = {
        "psro_coselfplay_phase_two": PSROPolicy_coselfplay_phase_two,
    }
    ...
    SecPass_new_agent_spec: AgentSpec = env.agent_spec["SecPass_new"]
    ...
    high_level_old_agent_spec: AgentSpec = env.agent_spec["high_level_old"]
    ...
    agent_spec_dict["SecPass_new"] = SecPass_new_agent_spec
    ...
    agent_spec_dict["high_level_old"] = high_level_old_agent_spec
...
def _load_low_level_policy(cfg: DictConfig, policy: PSROPolicy_coselfplay_phase_two):
    ...
    if cfg.get("SecPass_new_policy_checkpoint_path") is not None:
        policy.load_state_dict(torch.load(cfg.SecPass_new_policy_checkpoint_path), player="SecPass_new")
...
def _load_high_level_policy(cfg: DictConfig, policy: PSROPolicy_coselfplay_phase_two):
    """
    ...
    # assert cfg.get("actor_0_checkpoint_path")
    assert cfg.get("actor_1_checkpoints_directory_path")
    # assert cfg.get("high_level_policy_checkpoint_path")

    # # high_level
    # actor_dict_0 = torch.load(cfg.actor_0_checkpoint_path)

    # if actor_dict_0["actor_params"]["module"]["act_dist"]["operator"]["bias"].size()[-1] == 13: # 6 + 3 + 4 == 13
    #     original_weight = actor_dict_0["actor_params"]["module"]["act_dist"]["operator"]["weight"]
    #     weight_part1 = original_weight[:, :9, :] # (1, 9, 128)
    #     weight_part2 = original_weight[:, 9:, :] # (1, 4, 128)
    #     new_dim = torch.mean(original_weight[:, 6:9, :], dim=1).clone().unsqueeze(1) # (1, 1, 128)
    #     # new_dim = original_weight[:, 7, :].clone().unsqueeze(-1)
    #     expanded_weight = torch.cat([weight_part1, new_dim, weight_part2], dim=1)

    #     original_bias = actor_dict_0["actor_params"]["module"]["act_dist"]["operator"]["bias"]
    #     bias_part1 = original_bias[:, :9]  # [1,9]
    #     bias_part2 = original_bias[:, 9:]  # [1,4]
    #     new_bias = torch.mean(original_bias[:, 6:9], dim=1).clone().unsqueeze(1) # (1, 1)
    #     # new_bias = original_bias[:, 7].clone().unsqueeze(-1)
    #     expanded_bias = torch.cat([bias_part1, new_bias, bias_part2], dim=1)

    #     actor_dict_0["actor_params"]["module"]["act_dist"]["operator"]["weight"] = expanded_weight
    #     actor_dict_0["actor_params"]["module"]["act_dist"]["operator"]["bias"] = expanded_bias

    #     del actor_dict_0['actor_opt_params']
    
    # policy.policy_high_level.init_population_from_checkpoint(actor_dict_0, 0) # init population_0
    # policy.policy_high_level.init_population_from_checkpoint(actor_dict_0, 1) # init population_1 (placeholder)
    # print(f"Init high_level population_0 from checkpoint {cfg.actor_0_checkpoint_path}")
    ...
    for file_path in file_paths:
        if file_path == file_paths[0]:
            ...
            policy.policy_high_level_old.init_population_from_checkpoint(actor_dict_1, 1)
        else:
            actor_dict_1 = torch.load(file_path)
            policy.policy_high_level_old.population_1.add_actor(actor_dict_1)
        logging.info(f"Init high_level_old population_1 by appending actor_dict_1 from {file_path}")
    # init population_0 (placehoder)
    file_names = os.listdir(actor_1_directory_path)
    file_names = sorted(file_names)
    file_paths = [
        os.path.join(actor_1_directory_path, f)
    ...
    for file_path in file_paths:
        if file_path == file_paths[0]:
            ...
            policy.policy_high_level_old.init_population_from_checkpoint(actor_dict_0, 0)
        else:
            actor_dict_0 = torch.load(file_path)
            policy.policy_high_level_old.population_0.add_actor(actor_dict_0)
    policy.policy_high_level_old.set_actor_params_with_latest_policy(both_populations=True) # init actor network and actor_optimizer
...
@torch.no_grad()
def eval_win_rate(env: TransformedEnv, policy: PSROPolicy_coselfplay_phase_two):
    high_level_policy = policy.policy_high_level
    env.reset()
    env.eval()
    high_level_policy.eval_payoff = True
...
def get_new_payoffs(
    env: TransformedEnv,
    policy: PSROPolicy_coselfplay_phase_two,
    old_payoffs: Optional[np.ndarray],
):
    """
    compute missing payoff tensor entries via game simulations
    """
    high_level_policy_old = policy.policy_high_level_old
    high_level_policy = policy.policy_high_level
    
    m = len(high_level_policy.population_0)
    n = len(high_level_policy_old.population_1)
    
    new_payoffs = np.zeros(shape=(m, n))

    if old_payoffs is not None:
        assert (
            len(old_payoffs.shape) == 2
            and old_payoffs.shape[0] + 1 == m
            and old_payoffs.shape[1] == n
        )
        new_payoffs[:-1, :] = old_payoffs
        for i in range(n):
            high_level_policy.set_pure_strategy(idx_0=m-1, idx_1=m-1)
            high_level_policy_old.set_pure_strategy(idx_0=i, idx_1=i)
            wr = eval_win_rate(env=env, policy=policy)
            new_payoffs[m-1, i] = wr - (1 - wr)
    else:
        for i in range(m):
            for j in range(n):
                high_level_policy.set_pure_strategy(idx_0=i, idx_1=i)
                high_level_policy_old.set_pure_strategy(idx_0=j, idx_1=j)
                wr = eval_win_rate(env=env, policy=policy)
                new_payoffs[i, j] = wr - (1 - wr)
...
def log_heatmap(win_rate: np.ndarray):
    ...
    if win_rate.shape[0] < 5 and win_rate.shape[1] < 5:
...
def train(
    ...
    policy: PSROPolicy_coselfplay_phase_two = _get_psro_high_level_policy(cfg, env)
    _load_low_level_policy(cfg, policy) # load low-level policy
    _load_high_level_policy(cfg, policy) # load high-level policy

    high_level_old_policy = policy.policy_high_level_old
    high_level_old_policy.eval_payoff = True
    high_level_old_policy.current_player_id = 0
    
    high_level_policy = policy.policy_high_level
    high_level_policy.current_player_id = 0
    ...
    meta_policy_0 = np.array([1.0])
    # meta_policy_1 = np.array([0, 0, 0, 0, 1.0])
    meta_policy_1 = np.array([0, 1.0])
    assert len(meta_policy_0) == len(high_level_policy.population_0)
    assert len(meta_policy_1) == len(high_level_old_policy.population_1)
    ...
    # payoffs = np.zeros(shape=(1, 1)) # debug
    ...
    for i, data in enumerate(pbar):
        ...
        if result is not None: # update policy once with the high_level_policy sample buffer
            high_level_frames += cfg.task.env.num_envs * cfg.algo.train_every
            info.update(result) # update policy of the current player
            info.update({"high_level_frames": high_level_frames})
            wandb_run.log(info)
            print(OmegaConf.to_yaml({k: v for k, v in info.items() if isinstance(v, float)}))

        empirical_win_rate = get_emprical_win_rate(data, win_key=f"actor_0_wins", lose_key=f"actor_1_wins")
        ...
        if converged_indicator.converged():
            ...
            high_level_policy.append_actor(high_level_policy.current_player_id, share_population=True) # (c) Expand: add new policies to the population (If share_population == false and cnt_update % 2 == 1, the new policy is only used in the evaluation)
            high_level_policy.switch_current_player_id() # switch current player id

            if hasattr(policy, "state_dict"):
                # high_level
                ckpt_path = os.path.join(wandb_run.dir, f"checkpoint_{high_level_policy.population_0._module_idx}_high_level.pt")
        ...
        high_level_old_policy.sample_pure_strategy(meta_policy_1, meta_policy_1)
        # high_level_policy.sample_pure_strategy(meta_policy_0, meta_policy_0)
...
@hydra.main(version_base=None, config_path=CONFIG_PATH, config_name="train_coselfplay_phase_two")
