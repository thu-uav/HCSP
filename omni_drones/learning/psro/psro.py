...
from .utils import Population, high_level_uniform_policy, high_level_fixed_policy, high_level_fixed_policy_2
import wandb
import os
import contextlib
import numpy as np
...
class PSROPolicy(object):
    ...
    def make_critic(self):
        ...
        if self.cfg.critic_input == "state" and self.agent_spec.state_spec is not None:
            ...
            self.critic_out_keys = ["high_level_state_value"]
    ...
    def __call__(self, tensordict: TensorDict, deterministic: Optional[bool] = None):
        ...
        try:
            ...
            print(actor_output_0)
            print(actor_output_1)
            traceback.print_exc()
            raise e
    ...
    def update_actor(self, batch: TensorDict, agent_idx: int) -> Dict[str, Any]:
        ...
        assert advantages.shape == log_probs_new.shape == dist_entropy.shape # [E, numTeamMembers, 1]
    ...
    def update_critic(self, batch: TensorDict, agent_idx: int = None) -> Dict[str, Any]:
        ...
        values = self.value_op(critic_input)["high_level_state_value"]
        b_values = batch["high_level_state_value"]
    ...
    def train_op(self, tensordict: TensorDict):
        ...
        values = tensordict["high_level_state_value"]
        next_value = value_output["high_level_state_value"].squeeze(0)
    ...
    def set_actor_params_with_latest_policy(self, both_populations: bool = False):
        if both_populations:
            checkpoint = self.population_0.get_latest_policy_checkpoint()
            self.actor_params_0 = checkpoint["actor_params"]
            self.actor_opt_0 = torch.optim.Adam(self.actor_params_0.parameters(), lr=self.cfg.actor.lr)
            if "actor_opt_params" in checkpoint:
                self.actor_opt_0.load_state_dict(checkpoint["actor_opt_params"])
            logging.info("Set actor_params_0 and actor_opt_0 with the latest policy of population 0")

            checkpoint = self.population_1.get_latest_policy_checkpoint()
            self.actor_params_1 = checkpoint["actor_params"]
            self.actor_opt_1 = torch.optim.Adam(self.actor_params_1.parameters(), lr=self.cfg.actor.lr)
            if "actor_opt_params" in checkpoint:
                self.actor_opt_1.load_state_dict(checkpoint["actor_opt_params"])
            logging.info("Set actor_params_1 and actor_opt_1 with the latest policy of population 1")
        else:
            if self.current_player_id == 0:
                checkpoint = self.population_0.get_latest_policy_checkpoint()
                self.actor_params_0 = checkpoint["actor_params"]
    ...
    def set_actor_params_with_initial_random_policy(self):
        if self.current_player_id == 0:
            ...
            logging.info("Set actor_params_0 and actor_opt_0 with the initial random policy")
        else:
            ...
            logging.info("Set actor_params_1 and actor_opt_1 with the initial random policy")
    ...
    # def load_actor(self, actor_dict: dict, population_id: int):
    #     """
    #     Load actor and actor optimizer params in PSRO policy
    #     """
    #     if population_id == 0:
    #         self.actor_params_0 = actor_dict["actor_params"]
    #         self.actor_opt_0 = torch.optim.Adam(
    #             self.actor_params_0.parameters(), lr=self.cfg.actor.lr
    #         )
    #         self.actor_opt_0.load_state_dict(actor_dict["actor_opt_params"])
    #     elif population_id == 1:
    #         self.actor_params_1 = actor_dict["actor_params"]
    #         self.actor_opt_1 = torch.optim.Adam(
    #             self.actor_params_1.parameters(), lr=self.cfg.actor.lr
    #         )
    #         self.actor_opt_1.load_state_dict(actor_dict["actor_opt_params"])
    #     else:
    #         raise ValueError("Invalid population_id")
    
    # def init_population_from_checkpoint(self, actor_dict_0: dict, actor_dict_1: dict):
    #     """
    #     Init one initial actor for each population.
    #     """
    #     self.population_0 = Population(dir=os.path.join(
    #         wandb.run.dir, "population_0"), module=self.actor, initial_policy=actor_dict_0)
    #     self.population_1 = Population(dir=os.path.join(
    #         wandb.run.dir, "population_1"), module=self.actor, initial_policy=actor_dict_1)
        
    def init_population_from_checkpoint(self, actor_dict: dict, population_id: int):
        """
        Init one initial actor for one population.
        """
        if population_id == 0:
            self.population_0 = Population(dir=os.path.join(
                wandb.run.dir, "population_0"), module=self.actor, initial_policy=actor_dict)
        elif population_id == 1:
            self.population_1 = Population(dir=os.path.join(
                wandb.run.dir, "population_1"), module=self.actor, initial_policy=actor_dict)
...
def make_ppo_actor(cfg, observation_spec: TensorSpec, action_spec: TensorSpec):
    ...
    if isinstance(action_spec, MultiDiscreteTensorSpec): # important
        
        action_dim = action_spec.shape[-1]
        num_bins = action_spec.nvec.flatten(end_dim=-2)[0] # [2, 2]

        assert num_bins.shape[0] == action_dim
...
class PSROPolicy2(PSROPolicy):
    """
    PSRO for high-level policy
    """
    
    def _get_name(self):
        self.obs_name = ("agents", "high_level_observation")
        self.state_name = ("agents", "high_level_state")
        self.act_name = ("agents", "high_level_action")
        self.reward_name = ("agents", "high_level_reward")

    def _init_population(self, cfg):
        if cfg.get("initial_high_level_policy") == "fixed":
            self.population_0 = Population(dir=os.path.join(wandb.run.dir, "population_0"), module=self.actor, initial_policy=high_level_fixed_policy())
            self.population_1 = Population(dir=os.path.join(wandb.run.dir, "population_1"), module=self.actor, initial_policy=high_level_fixed_policy())
        elif cfg.get("initial_high_level_policy") == "fixed_2":
            self.population_0 = Population(dir=os.path.join(wandb.run.dir, "population_0"), module=self.actor, initial_policy=high_level_fixed_policy_2())
            self.population_1 = Population(dir=os.path.join(wandb.run.dir, "population_1"), module=self.actor, initial_policy=high_level_fixed_policy_2())
        elif cfg.get("initial_high_level_policy") == "random":
            self.population_0 = Population(dir=os.path.join(wandb.run.dir, "population_0"), module=self.actor, initial_policy=high_level_uniform_policy())
            self.population_1 = Population(dir=os.path.join(wandb.run.dir, "population_1"), module=self.actor, initial_policy=high_level_uniform_policy())
        else:
            raise ValueError("Wrong initial_high_level_policy")
...
class PSROPolicy3(PSROPolicy):
    """
    PSRO for high-level old policy
    """
    
    def _get_name(self):
        self.obs_name = ("agents", "high_level_observation")
        self.state_name = ("agents", "high_level_state")
        self.act_name = ("agents", "high_level_old_action")
        self.reward_name = ("agents", "high_level_reward")

    def _init_population(self, cfg):
        if cfg.get("initial_high_level_policy") == "fixed":
            self.population_0 = Population(dir=os.path.join(wandb.run.dir, "population_0_old"), module=self.actor, initial_policy=high_level_fixed_policy())
            self.population_1 = Population(dir=os.path.join(wandb.run.dir, "population_1_old"), module=self.actor, initial_policy=high_level_fixed_policy())
        elif cfg.get("initial_high_level_policy") == "fixed_2":
            self.population_0 = Population(dir=os.path.join(wandb.run.dir, "population_0_old"), module=self.actor, initial_policy=high_level_fixed_policy_2())
            self.population_1 = Population(dir=os.path.join(wandb.run.dir, "population_1_old"), module=self.actor, initial_policy=high_level_fixed_policy_2())
        elif cfg.get("initial_high_level_policy") == "random":
            self.population_0 = Population(dir=os.path.join(wandb.run.dir, "population_0_old"), module=self.actor, initial_policy=high_level_uniform_policy())
            self.population_1 = Population(dir=os.path.join(wandb.run.dir, "population_1_old"), module=self.actor, initial_policy=high_level_uniform_policy())
        else:
            raise ValueError("Wrong initial_high_level_policy")
    ...
    def init_population_from_checkpoint(self, actor_dict: dict, population_id: int):
        ...
        if population_id == 0:
            self.population_0 = Population(dir=os.path.join(
                wandb.run.dir, "population_0_old"), module=self.actor, initial_policy=actor_dict)
        elif population_id == 1:
            self.population_1 = Population(dir=os.path.join(
                wandb.run.dir, "population_1_old"), module=self.actor, initial_policy=actor_dict)