...
from omni_drones.learning import MAPPOPolicy, PSROPolicy, PSROPolicy2, MAPPOPolicy_mask


class PSROPolicy_high_low_level(object):
    ...
    def __init__(self, cfg, agent_spec_dict: Dict[str, AgentSpec], device, num_envs) -> None:
        ...
        self.policy_FirstPass = MAPPOPolicy_mask(cfg=cfg, agent_spec=FirstPass_agent_spec, device=device, mask_name=('stats', 'FirstPass_train_flag'))
        cfg.agent_name = "FirstPass_hover"
        self.policy_FirstPass_hover = MAPPOPolicy_mask(cfg=cfg, agent_spec=FirstPass_hover_agent_spec, device=device, mask_name=('stats', 'FirstPass_hover_train_flag'))
        ...
        self.policy_SecPass = MAPPOPolicy_mask(cfg=cfg, agent_spec=SecPass_agent_spec, device=device, mask_name=('stats', 'SecPass_train_flag'))
        cfg.agent_name = "SecPass_hover"
        self.policy_SecPass_hover = MAPPOPolicy_mask(cfg=cfg, agent_spec=SecPass_hover_agent_spec, device=device, mask_name=('stats', 'SecPass_hover_train_flag'))

        cfg.agent_name = "Att_goto"
        self.policy_Att_goto = MAPPOPolicy(cfg=cfg, agent_spec=Att_goto_agent_spec, device=device)
        cfg.agent_name = "Att"
        self.policy_Att = MAPPOPolicy_mask(cfg=cfg, agent_spec=Att_agent_spec, device=device, mask_name=('stats', 'Att_train_flag'))
        cfg.agent_name = "Att_hover"
        self.policy_Att_hover = MAPPOPolicy_mask(cfg=cfg, agent_spec=Att_hover_agent_spec, device=device, mask_name=('stats', 'Att_hover_train_flag'))
        ...
        self.buffer = {}
        self.buffer[0] = torch.zeros((self.num_envs, self.train_every, 2, 98), device=device)
        ...
        self.buffer[5] = torch.zeros((self.num_envs, self.train_every, 2, 98), device=device) # next_obs
        ...
        self.high_level_is_fixed = cfg.high_level_is_fixed
    ...
    def train_op(self, tensordict: TensorDict):
        if self.high_level_is_fixed:
            high_level_policy_train_info = None
        else:
            high_level_policy_train_info = self._high_level_policy_train_op(tensordict)

        FirstPass_policy_train_info = self.policy_FirstPass.train_op(tensordict)
        # FirstPass_hover_policy_train_info = self.policy_FirstPass_hover.train_op(tensordict)

        # SecPass_policy_train_info = self.policy_SecPass.train_op(tensordict)
        # SecPass_hover_policy_train_info = self.policy_SecPass_hover.train_op(tensordict)

        # Att_policy_train_info = self.policy_Att.train_op(tensordict)
        # Att_hover_policy_train_info = self.policy_Att_hover.train_op(tensordict)

        return {
            "FirstPass": FirstPass_policy_train_info,
            # "FirstPass_hover": FirstPass_hover_policy_train_info,
            # "SecPass": SecPass_policy_train_info,
            # "SecPass_hover": SecPass_hover_policy_train_info,
            # "Att": Att_policy_train_info,
            # "Att_hover": Att_hover_policy_train_info,
            "high_level": high_level_policy_train_info
        }
    
    def _high_level_policy_train_op(self, tensordict: TensorDict):
        '''
